...
if (preg_match("/\bindex.php\b/i", $_SERVER['REQUEST_URI'])) {
    ...
    switch ($act) {
        ...
        default:
            ...
            $sqld = "SELECT * FROM gl_satuan_barang WHERE hapus = 0 AND nama_satuan LIKE '%$gname%'";
                        ...
                        <div class=\"box-body\">
                            <form role=\"form\" class=\"form-horizontal\" method=\"get\">
                                ...
                                <div class=\"form-group\">
                                    <label class=\"col-sm-2 control-label text-left\">Cari Satuan Barang</label>
                                    <div class=\"col-sm-3\">
                                        <input type=\"text\" class=\"form-control\" name=\"gname\" value=\"$gname\" placeholder=\"Nama Satuan Barang\">
                                    </div>
                                </div>
                                <div class=\"form-group\">
            ...
            break;
        // Menambah atau Mengedit Data Satuan Barang
        case "input":
            if ($gket == "tambah" && $lmn[$idsmenu][1] == 1) {
                $judul = "Tambah Data Satuan Barang";
            } elseif ($gket == "edit" && $lmn[$idsmenu][2] == 1) {
                $judul = "Edit Data Satuan Barang";

                $sqld = "SELECT * FROM gl_satuan_barang WHERE id = ? AND hapus = 0";
                        ...
                        <div class='box box-primary'>
                            ...
                            <div class=\"box-body\">
                                <form role=\"form\" class=\"form-horizontal\" method=\"post\" action=\"$link_back&act=proses&gket=$gket\">
                                    <div class=\"form-group\">
                                        <label class=\"col-sm-2 text-left\"><b>Nama Satuan Barang</b></label>
                                        <div class=\"col-sm-3\">
                                            <input type=\"text\" class=\"form-control\" name=\"nama_satuan\" value=\"$nama_satuan\" required />
                                        </div>
                                    </div>
                                    <div class=\"form-group\">
        ...
        // Proses Menambah atau Mengedit Data Satuan Barang
        case "proses":
            if (count($_POST) > 0) {
                $nama_satuan = mysqli_real_escape_string($db_result, $_POST['nama_satuan']);
                $status = mysqli_real_escape_string($db_result, $_POST['status']);

                if ($gket == "tambah" && $lmn[$idsmenu][1] == 1) {
                    $inp = "INSERT INTO gl_satuan_barang (nama_satuan, status, hapus, tgl_insert, user_update)
                            VALUES (?, ?, 0, NOW(), 'admin')";
                    $stmt = mysqli_prepare($db_result, $inp);
                    mysqli_stmt_bind_param($stmt, "si", $nama_satuan, $status);
                    ...
                    $inp = "UPDATE gl_satuan_barang SET 
                            nama_satuan = ?, 
                            status = ?, 
                            tgl_update = NOW(), 
                            user_update = 'admin' 
                            WHERE id = ?";
                    $stmt = mysqli_prepare($db_result, $inp);
                    mysqli_stmt_bind_param($stmt, "sii", $nama_satuan, $status, $pid);
                    $upd = mysqli_stmt_execute($stmt);
        ...
        case "hapus":
            if ($lmn[$idsmenu][3] == 1) {
                $inp = "UPDATE gl_satuan_barang SET hapus = 1, tgl_update = NOW(), user_update = 'admin' WHERE id = ?";
                $stmt = mysqli_prepare($db_result, $inp);
                mysqli_stmt_bind_param($stmt, "i", $gid);
                $upd = mysqli_stmt_execute($stmt);
